...
WITH ranked_duplicates AS (
  SELECT 
    id,
    normalized_symbol,
    ROW_NUMBER() OVER (PARTITION BY normalized_symbol ORDER BY created_at ASC) as rn,
    COUNT(*) OVER (PARTITION BY normalized_symbol) as dup_count
...
duplicate_groups AS (
  SELECT 
    normalized_symbol,
    MAX(dup_count) as total_count
  FROM ranked_duplicates
  WHERE dup_count > 1
  GROUP BY normalized_symbol
),
keepers AS (
  SELECT rd.id, rd.normalized_symbol, dg.total_count
  FROM ranked_duplicates rd
  JOIN duplicate_groups dg ON rd.normalized_symbol = dg.normalized_symbol
  WHERE rd.rn = 1
)
UPDATE pending_ticker_mappings p
SET 
  seen_count = k.total_count,
  updated_at = now()
FROM keepers k
WHERE p.id = k.id;

-- Delete duplicate records (keep only the earliest one)
...
-- Now create unique index on normalized_symbol for pending records
...
BEGIN
  ...
  LOOP
    ...
    SET status = 'approved',
        auto_approved = true,
        reviewed_at = now(),
        validation_notes = 'Auto-approved: ' || COALESCE(pending_rec.match_type, 'unknown') || ' confidence ' || COALESCE(pending_rec.confidence_score, 0)
  ...
  WITH rejected AS (
    ...
    SET status = 'rejected',
        reviewed_at = now(),
        validation_notes = 'Auto-rejected: Low confidence with conflicts'
    WHERE status = 'pending'
