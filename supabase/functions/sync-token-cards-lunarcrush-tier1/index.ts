...
async function fetchWithRetry(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    if (response.status === 429) {
      ...
      console.log(`[tier1] Rate limited (429), waiting ${waitTime/1000}s before retry ${attempt + 1}/${maxRetries}`);
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[tier1] Starting sync for TOP 1000 tokens...');

    // Fetch only first 1000 tokens (1 API call)
    const LIMIT = 1000;
    const url = `https://lunarcrush.com/api4/public/coins/list/v1?limit=${LIMIT}&offset=0&sort=market_cap_rank&order=asc`;
    ...
    if (!response || !response.ok) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: `API error: ${response?.status || 'no response'}`
    ...
    console.log(`[tier1] Fetched ${allCoins.length} top tokens`);
    ...
    // Load existing cards with pagination
    ...
    while (true) {
      ...
      if (fetchError) throw new Error(`Failed to fetch token_cards: ${fetchError.message}`);
    ...
    for (const card of allExistingCards) {
      ...
      for (const addr of extractAddresses(card.contracts)) {
    ...
    for (const coin of dedupedCoins) {
      try {
        ...
        // Match priority: lunarcrush_id > contract > symbol
        ...
        if (!matchedCard && symbol) {
          ...
          matchedCard = symbolMatches.find(c => !c.lunarcrush_id);
        }
        ...
        errors++;
        console.error(`[tier1] Error processing ${coin.symbol}:`, e.message);
      }
    }
    ...
    for (let i = 0; i < updates.length; i += BATCH_SIZE) {
      ...
      for (const update of batch) {
        ...
        if (error) { console.error(`[tier1] Update error:`, error.message); errors++; }
      }
    }
    ...
    for (let i = 0; i < inserts.length; i += BATCH_SIZE) {
      ...
      const { error } = await supabase.from('token_cards').upsert(batch, { onConflict: 'canonical_symbol', ignoreDuplicates: false });
      if (error) { console.error(`[tier1] Insert batch error:`, error.message); errors += batch.length; }
    }

    const duration = Date.now() - startTime;
    console.log(`[tier1] Sync complete in ${duration}ms - Updated: ${updated}, Created: ${created}, Errors: ${errors}`);
    ...
    console.error('[tier1] Fatal error:', error);
