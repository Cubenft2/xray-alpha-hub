...
// Helper to log API call to external_api_calls table - logs each attempt
...
async function fetchWithRetry(
  ...
  console.error(`[sync-token-cards-lunarcrush] Max retries exceeded for rate limiting after ${totalAttempts} attempts`);
...
serve(async (req) => {
  ...
  try {
    ...
    for (let offset = 0; offset < 3000; offset += LIMIT) {
      ...
      console.log(`[sync-token-cards-lunarcrush] Fetch at offset ${offset} completed with ${totalAttempts} attempt(s)`);
