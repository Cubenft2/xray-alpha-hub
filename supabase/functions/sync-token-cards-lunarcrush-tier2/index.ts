...
async function fetchWithRetry(url: string, headers: Record<string, string>, maxRetries = 3): Promise<Response | null> {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    if (response.status === 429) {
      ...
      console.log(`[tier2] Rate limited, waiting ${waitTime/1000}s...`);
...
async function logApiCall(supabase: any, apiName: string, functionName: string, success: boolean, errorMessage?: string) {
  try {
    await supabase.from('external_api_calls').insert({
      api_name: apiName, function_name: functionName, call_count: 1, success, error_message: errorMessage || null,
    });
  } catch (e) { console.error('[tier2] Failed to log API call:', e); }
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('[tier2] Starting sync for tokens 1001-2000...');

    // Fetch tokens 1001-2000 (offset 1000, limit 1000)
    const url = `https://lunarcrush.com/api4/public/coins/list/v1?limit=1000&offset=1000&sort=market_cap_rank&order=asc`;
    const headers: Record<string, string> = { 'Accept': 'application/json' };
    if (lunarcrushKey) headers['Authorization'] = `Bearer ${lunarcrushKey}`;

    const response = await fetchWithRetry(url, headers);
    await logApiCall(supabase, 'lunarcrush', 'sync-token-cards-lunarcrush-tier2', response?.ok ?? false, 
      response?.ok ? undefined : `HTTP ${response?.status || 'no response'}`);
    
    if (!response || !response.ok) {
      console.error(`[tier2] LunarCrush API error: ${response?.status || 'no response'}`);
    ...
    console.log(`[tier2] Fetched ${allCoins.length} tokens (ranks 1001-2000)`);
    ...
    for (const coin of dedupedCoins) {
      ...
      } catch (e) { errors++; console.error(`[tier2] Error processing ${coin.symbol}:`, e.message); }
    }
    ...
    for (let i = 0; i < updates.length; i += BATCH_SIZE) {
      for (const update of updates.slice(i, i + BATCH_SIZE)) {
        ...
        if (error) { console.error(`[tier2] Update error:`, error.message); errors++; }
      }
    }
    for (let i = 0; i < inserts.length; i += BATCH_SIZE) {
      ...
      if (error) { console.error(`[tier2] Insert error:`, error.message); errors += BATCH_SIZE; }
    }

    const duration = Date.now() - startTime;
    console.log(`[tier2] Sync complete in ${duration}ms - Updated: ${updated}, Created: ${created}, Errors: ${errors}`);

    return new Response(JSON.stringify({
      success: true, tier: 2, range: '1001-2000',
      stats: { fetched: allCoins.length, updated, created, errors, duration_ms: duration }
    }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });

  } catch (error) {
    console.error('[tier2] Fatal error:', error);
