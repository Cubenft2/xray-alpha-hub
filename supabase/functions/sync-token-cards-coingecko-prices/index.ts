...
serve(async (req) => {
  ...
  try {
    ...
    for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
      ...
      const url = `${baseUrl}/coins/markets?vs_currency=usd&ids=${idsParam}&order=market_cap_desc&per_page=250&page=1&sparkline=false&price_change_percentage=1h,24h,7d,30d`;
      ...
      try {
        ...
        for (const coin of marketData) {
          ...
          updates.push({
            ...
            coingecko_fdv: coin.fully_diluted_valuation,
            // Extended price changes (1h, 7d, 30d)
            change_1h_pct: coin.price_change_percentage_1h_in_currency,
            change_7d_pct: coin.price_change_percentage_7d_in_currency,
            change_30d_pct: coin.price_change_percentage_30d_in_currency,
            // Supply data
