...
Deno.serve(async (req) => {
  ...
  try {
    console.log('🌆 Evening Brief wrapper invoked');
    ...
    console.log('✅ Evening brief generated successfully');
    return new Response(
      JSON.stringify({ success: true, data }),
      { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  } catch (error) {
    console.error('❌ Unexpected error in evening wrapper:', error);
